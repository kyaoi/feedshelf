...
- community source でも source 自身または first-party help / docs で feed path が確認できる topic feed は `enabled=true` にしてよく、Zenn topic や Reddit subreddit RSS のような documented topic feed は cautious default の範囲で有効化してよい
- ただし `Zenn Feed` / `Hacker News` / `r/programming` のような broad feed や、science 棚を重くしすぎる source、evidence の薄い topic feed は `enabled=false` のまま保持してよい
