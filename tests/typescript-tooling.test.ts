...
test('FS-DOCS-42 fuzzy dedupe scope observability docs split stays aligned across PLAN, SPEC, DECISIONS, and TRACEABILITY', () => {
  const plan = fs.readFileSync(
    path.resolve(__dirname, '..', 'PLAN.md'),
    'utf8',
  );
  ...
  assert.match(plan, /FS-DOCS-42/);
  assert.match(plan, /internal match-scope surfacing/);
  assert.match(plan, /scopeKind/);
  assert.match(
    spec,
    /10\.5\.28 Post-v1 fuzzy dedupe scope observability docs split/,
  );
  assert.match(spec, /scopeKind/);
  assert.match(spec, /sourceFamily/);
  assert.match(decisions, /D-174/);
  assert.match(decisions, /human reviewer/);
  assert.match(traceability, /FS-188/);
  assert.match(
    traceability,
    /accept-reject key change \/ public JSON \/ state mutation/,
  );
});
...
test('FS-DATA-14 fuzzy dedupe false-positive reject implementation stays aligned across PLAN, SPEC, DECISIONS, TRACEABILITY, and tests', () => {
  const plan = fs.readFileSync(
    path.resolve(__dirname, '..', 'PLAN.md'),
    'utf8',
  );
  ...
  assert.match(plan, /FS-DATA-14/);
  assert.match(plan, /`--fuzzy-reject-file`/);
  assert.match(
    spec,
    /10\.5\.10 Post-v1 fuzzy dedupe false-positive reject implementation/,
  );
  assert.match(spec, /operator-authored な JSON reject list/);
  assert.match(decisions, /D-156/);
  assert.match(decisions, /`--fuzzy-reject-file <path>`/);
  assert.match(traceability, /FS-170/);
  assert.match(contracts, /export interface FuzzyDedupeRejectEntry/);
  assert.match(dedupeSource, /fuzzyRejectEntries/);
  assert.match(dedupeSource, /isFuzzyMergeRejected/);
  assert.match(runSource, /--fuzzy-reject-file/);
  assert.match(runSource, /loadFuzzyRejectEntries/);
  assert.match(updateSource, /--fuzzy-reject-file/);
  assert.match(updateSource, /loadFuzzyRejectEntries/);
  assert.match(
    loadFeedsTest,
    /runPipeline suppresses fuzzy merges when --fuzzy-reject-file is provided/,
  );
  assert.match(
    updateWorkflowTest,
    /runUpdatePipeline suppresses fuzzy merges when --fuzzy-reject-file is provided/,
  );
});
...
test('FS-DATA-17 fuzzy dedupe manual-review HTML implementation stays aligned across PLAN, SPEC, DECISIONS, TRACEABILITY, and tests', () => {
  const plan = fs.readFileSync(
    path.resolve(__dirname, '..', 'PLAN.md'),
    'utf8',
  );
  ...
  assert.match(plan, /FS-DATA-17/);
  assert.match(plan, /`--fuzzy-review-html-file`/);
  assert.match(
    spec,
    /10\.5\.17 Post-v1 fuzzy dedupe manual-review HTML implementation/,
  );
  assert.match(spec, /single-file の self-contained HTML artifact/);
  assert.match(decisions, /D-163/);
  assert.match(decisions, /`--fuzzy-review-html-file <path>`/);
  assert.match(traceability, /FS-177/);
  assert.match(contracts, /fuzzyReviewHtmlPath: string \| null;/);
  assert.match(runSource, /--fuzzy-review-html-file/);
  assert.match(runSource, /buildFuzzyReviewHtml/);
  assert.match(updateSource, /--fuzzy-review-html-file/);
  assert.match(
    updateSource,
  ...
  assert.match(
    loadFeedsTest,
    /runPipeline writes fuzzy review HTML when --fuzzy-review-html-file is provided/,
  );
  assert.match(
    updateWorkflowTest,
    /runUpdatePipeline writes fuzzy review HTML when --fuzzy-review-html-file is provided/,
  );
});
