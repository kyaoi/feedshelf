...
- [ ] `FS-IA-00` Phase 6 の docs-first planning を行い、読み物棚寄りの体験・無料運用制約・作業順序を docs に固定する
- [ ] `FS-IA-01` `data/shelves.yaml` と `data/feeds.json` の責務分離を仕様化する
- [ ] `FS-IA-02` shelf route policy、reserved ids、GitHub Pages 単一サイト前提の URL 設計を固定する
- [ ] `FS-IA-03` Phase 6 の公開 JSON / search index / tag summary の契約を定義する

完了条件:
- 棚 (`shelf`)・source・tag・search の責務分割が docs 上で迷いなく説明できる
- `shelves.yaml` / `feeds.json` / canonical article object / public JSON の関係が追跡できる
- route policy と reserved ids が固定され、後続実装で path を場当たり的に増やさなくてよい

#### 6-B. Shelf-first UI / UX

- [ ] `FS-UX-00` ルート `/` を「棚カタログページ」として再設計する
- [ ] `FS-UX-01` `/<shelfId>/` 棚ページの構成（概要・注目・新着・source導線）を設計する
- [ ] `FS-UX-02` 記事カードの見た目を discovery-first に見直し、タイトル・タグ・source・余白を再設計する
- [ ] `FS-UX-03` `sources` 導線を主役から補助導線へ位置づけ直し、棚・タグ・検索と矛盾しない UI にする
- [ ] `FS-UX-04` モバイル表示、loading / empty / error、long tag / long title の崩れを改善する
...
- `FS-IA-00` から開始し、Phase 6 全体の docs-first planning を `PLAN / SPEC_V1 / DECISIONS / TRACEABILITY` に同期する
...
- `data/shelves.yaml` は棚定義のみを持ち、`data/feeds.json` は source 定義・`shelfIds[]`・手動 `tags[]` を持つ。custom path は v1 では持たない
- `shelfIds` は常に配列とし、source が複数棚へ属してよいことを正式仕様候補とする
- source の `tags` は curator-managed な手動タグ、`entryTags` は RSS / Atom metadata から best-effort で抽出する記事タグとして分離する
