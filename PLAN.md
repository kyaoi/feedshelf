...
- Phase 6 実装後の維持では、`feeds.json.shelfIds[]` が `shelves.yaml` に存在する棚だけを参照していることを pipeline が fail-fast で検証し、rename / delete 後に orphaned source / article export を出さないことも evidence に含める
