...
`FS-TS-02` で実際に入れるもの:
- `src/shared/contracts.ts` を追加し、`FeedDefinition` / `CanonicalArticle` / 公開 JSON shape を集約する
- `scripts/pipeline/*.ts` に型付き実装を追加する
- 既存の `.js` ファイルは互換 wrapper として一時的に残す
- test 実行は `tsx --test tests` とし、JS test fixture から TS 実装を読めるようにする
...
- browser が読む asset path は `public/assets/app.js` のまま維持する
- source-of-truth は `src/web/app.ts` に置き、公開用 JS は dedicated emit config から再生成できるようにする

`FS-TS-03` で実際に入れるもの:
- `src/web/app.ts` を追加し、ホーム / カテゴリ / 媒体一覧の UI ロジックを TS へ移す
- `public/assets/app.js` は HTML 互換の browser asset path として残す
- `tsconfig.web.json` と `build:web-ui` を追加し、TS source から browser asset を再生成できるようにする
- HTML の `<script src>` や `public/*.html` の導線は変えない
...
実装方針:
- tests は `.ts` へ移し、`tsx --test tests/*.test.ts` を基準にする
- lint は `scripts/lint.ts` を正本とし、残る `.js` wrapper / generated asset の syntax と基本契約だけを確認する
- verify では `src/web/app.ts` から再生成した browser asset と checked-in `public/assets/app.js` の一致を確認する
- 実行入口の変更は docs と verification checklist を同時更新する

`FS-TS-04` で実際に入れるもの:
- `tests/*.test.ts` と `scripts/lint.ts` への移行
- `verify:web-ui` script と `scripts/verifyWebBuild.ts` の追加
- `pnpm run ci` / `just ci` へ browser asset verify を組み込む
- `tsconfig.web.json` を verify 可能な設定へ補正する

### 5. `FS-TS-05`: strictness を段階的に引き上げる

今回の実装:
- `strict: true` を有効化する
- `DOM.Iterable` を追加し、URL/Fetch 系 DOM API を strict mode でも扱えるようにする
- tests と pipeline 内の暗黙 `any` / regex capture / optional capture を最小差分で明示化する
...
実装方針:
- 先に `strict: true` を成立させ、その後に追加 strictness を個別タスクで上げる
...
- `tsx scripts/lint.ts` または同等 gate
