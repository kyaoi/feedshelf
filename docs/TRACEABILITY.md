...
| FS-076 | tag / search / shelf を支える公開 JSON 契約を追加定義できる | `docs/SPEC_V1.md` / `PLAN.md` / `src/shared/contracts.ts` / `scripts/pipeline/buildPublicExports.ts` / `tests/load-feeds.test.ts` | `tags.json` と tag-aware な `articles.json` / `sources.json` / `meta.json` 契約が tests と実装で確認できる |
